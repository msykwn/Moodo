...
- `/Users/piyo/repository/Moodo/backend/tasks.json` … タスク一覧。各タスクの title・期限（due_date）・見積もり時間（estimate_minutes・分）・めんどくさレベル（bother_level）・重要度（importance）・詳細（description）を持つ
- `/Users/piyo/repository/Moodo/backend/mood.json` … 現在の気分（mood）と空き時間（available_minutes・分）

## スコアリングの考え方

- 気分が良いほど、めんどくさいタスクにも取り組める
- 空き時間（available_minutes）に収まるタスクを優先する
- 期限が近いほど優先度が上がる
