...
- 初期実装後、タスク編集時に `today_flag` が消えるバグ（`updateTask` が `today_flag` を含まない問題）が発覚し修正した
- モーダルのフェードクローズ実装時、ソートロジックが重複していたことが判明し整理した
- ピックアップエリア（画面下部固定・`z-index: 100`）とモーダルが重なり保存ボタンが押せなくなる問題が後から発覚。`modal-backdrop` に `z-index: 200` を追加して対処した

## この決定によって生じる影響

- ピックアップエリアより上のレイヤーにモーダルを表示するため、`z-index` の管理が必要になった（modal: 200, pickup: 100）
- `today_flag` の変更はモーダル経由のみのため、タスクカード上でのワンクリックトグルは現状できない（意図的な制約）
- `completed_tasks.json` はGit管理外とした（`.gitignore` 追加）
